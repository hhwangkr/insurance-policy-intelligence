...
- **pytest** — protect **product/document-agnostic invariants**: schemas, filter semantics, deterministic IDs, validation rules, and inspection output shape. Avoid encoding every insurer-specific section title in code.
...
- **Quality / inspection reports** — first-line tools for **newly added** documents (`inspect_*` CLIs, markdown under `data/processed/reports/`, citation-context and grounded-answer inspection). Gitignored artifacts; not the contract for every future policy.
- **Manual review** — answer **faithfulness** until a reliable automated judge exists.
...
- grounded answer validation rejects invented citation IDs
- inspection reports separate **cited** vs **uncited** retrieved contexts
...
[`data/eval/retrieval_queries.yaml`](../data/eval/retrieval_queries.yaml) drives `evaluate_retrieval`. It is **intentionally curated and document-specific** for the demo policies. It is **not** a universal contract for every future PDF.

## Grounded answer inspection

`inspect_answer` and related modules support **manual** cited-vs-uncited review. They complement **citation ID validation**, which does not judge prose faithfulness.
...
8. Inspect **generated reports** (retrieval eval markdown, chunk/ingestion reports, optional `inspect_answer`) for the new document.
