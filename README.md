...
- **Generation layer:** language-neutral grounded prompt; **structured** citation validation (default) on allowed **`citations_used`**; optional **`--require-inline-citations`** for strict **`[C1]`**-in-text mode; **`render_grounded_answer_with_citations`** (deterministic **`[Cn]`** from **`citations_used`**) and **`render_citation_summary`** (section title + pages from the bundle); **`GroundedAnswer`**, **`LLMProvider`**, **`generate_grounded_answer`** (citation-aware JSON Schema for Ollama `format`), **`provider_registry`**, debug CLIs **`build_answer_prompt`** / **`generate_answer`** (adds **`rendered_answer`** / **`citation_summary`** to printed JSON; **`--fail-on-invalid`** defaults on for Ollama; no paid APIs)
